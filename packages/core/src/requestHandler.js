    ...
    co(fetchReqData)
      ...
      .then(
        co.wrap(function* () {
          ...
          ['protocol', 'requestOptions', 'requestData', 'response'].forEach((key) => {
      ...
      .then(
        co.wrap(function* (userConfig) {
          if (userConfig.response) {
            ...
            const remoteResponse = yield fetchRemoteResponse(
              userConfig.protocol,
              userConfig.requestOptions,
              userConfig.requestData,
              {
      ...
      .then(
        co.wrap(function* (responseData) {
          if (responseData._directlyPassToRespond) {
            ...
            // TODO: err etimeout
            return (
              (yield userRule.beforeSendResponse(Object.assign({}, requestDetail), Object.assign({}, responseData))) ||
              responseData
            );
          }
        })
...
class RequestHandler {
  ...
  constructor(config, rule, recorder) {
    ...
    this.connectReqHandler = getConnectReqHandler.apply(reqHandlerCtx, [
      userRule,
      recorder,
      reqHandlerCtx.httpsServerMgr,
    ]);
  }
}
