...
class RecordRequestDetail extends React.Component {
  ...
  getReqBodyDiv() {
    ...
    const reqDownload = (
      <a href={`/fetchReqBody?id=${recordDetail.id}&_t=${Date.now()}`} target="_blank">
        download
      </a>
    );
    ...
    return <div className={Style.reqBody}>{getReqBodyContent()}</div>;
  }
    ...
    return (
      <div>
        <div className={Style.section}>
          ...
          <ul className={Style.ulItem}>
            <li className={Style.liItem}>
              <strong>CURL:</strong>
              <span>
                <a href="javascript:void(0)" onClick={this.copyCurlCmd}>
                  copy as CURL
                </a>
              </span>
            </li>
        ...
        <div className={Style.section}>
          ...
          <div className={CommonStyle.whiteSpace10} />
          <ul className={Style.ulItem}>{this.getLiDivs(reqHeader)}</ul>
        </div>
