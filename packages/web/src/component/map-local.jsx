  ...
  getFormDiv() {
    ...
    return (
      <div className={Style.form}>
        <Form vertical>
          <FormItem label="Keyword">{keywordProps(<Input placeholder="The pattern to map" />)}</FormItem>
          <FormItem label="Local file">{localProps(<Input placeholder="Local file for the mapped url" />)}</FormItem>
        </Form>
      </div>
    );
  }
  ...
  getMappedConfigDiv() {
        ...
        <li key={index}>
          <div>
            <div className={Style.mappedKeyDiv}>
              ...
              <a href="javascript:void(0)" onClick={this.removeMappedConfig.bind(this, index)}>
                Remove
              </a>
            </div>
            <div className={Style.mappedLocal}>{item.local}</div>
          </div>
        </li>
      );
    });
    ...
    return (
      <div className={Style.mappedConfigWrapper}>
        ...
        <ul className={Style.mappedList}>{mappedLiDiv}</ul>
      </div>
    );
  }
    ...
    return (
      <ResizablePanel onClose={this.onClose} visible={panelVisible}>
        <div className={Style.mapLocalWrapper}>
          <div className={Style.title}>Map Local</div>
          ...
          <div className={Style.treeWrapper}>
            <Tree loadData={this.loadTreeNode} onSelect={this.onNodeSelect}>
              {treeNodes}
            </Tree>
          </div>
          <div className={Style.operations}>
            <Button type="primary" onClick={this.addMappedConfig}>
              Add
            </Button>
          </div>
        </div>
