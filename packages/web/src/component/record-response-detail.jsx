...
class RecordResponseDetail extends React.Component {
  ...
  getResBodyDiv() {
    ...
    switch (recordDetail.type) {
      ...
      default: {
        if (!recordDetail.resBody && recordDetail.ref) {
          reqBodyDiv = (
            <a href={recordDetail.ref} target="_blank">
              {recordDetail.fileName}
            </a>
          );
        }
        break;
    ...
    return <div className={Style.resBody}>{reqBodyDiv}</div>;
  }
    ...
    return (
      <div>
        ...
        <div className={Style.section}>
          ...
          <div className={CommonStyle.whiteSpace10} />
          <ul className={Style.ulItem}>{this.getLiDivs(recordDetail.resHeader)}</ul>
        </div>
