...
class RecordRow extends React.Component {
  ...
  getMethodDiv(item) {
    ...
    return (
      <div className={CommonStyle.topAlign}>
        <div>{item.method}</div> {item.protocol === 'https' ? httpsIcon : null}
      </div>
    );
  }
  ...
  render() {
    ...
    return (
      <tr className={this.props.className} onClick={this.props.detailHandler.bind(this, data.id)}>
        ...
        <td className={Style.path} title={data.path}>
          {data.path}
        </td>
        <td className={Style.mime} title={data.mime}>
          {data.mime}
        </td>
